...
export default function BookPage() {
  ...
  return (
    <ProtectedRoute>
      <section className="p-8 grid grid-cols-1 md:grid-cols-3 gap-6 bg-[#816678]">
        {loading && (
          <p className="text-neutral-300">Loading books…</p>
        )}
        ...
        {!loading && !error && books.length === 0 && (
          <p className="text-neutral-300">No books available.</p>
        )}
        ...
        {!loading && !error && books.map((book) => (
          ...
          >
            <article>
              ...
              <p className="text-sm text-neutral-300 line-clamp-3">{book.description}</p>
            </article>
          </Link>
        ))}
      </section>
